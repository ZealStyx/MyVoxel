...
// Rim lighting (physics bodies only)
...
void main() {
    ...
    // Rim lighting (physics bodies get a rim highlight)
    float rim = 1.0 - max(dot(V, N), 0.0);
    rim = pow(rim, u_rimPower);
    vec3 rimContrib = u_rimColor * rim;

    vec3 color = ambient + Lo * shadow + u_emission + rimContrib * albedo;
    ...
    gl_FragColor = vec4(color, u_emissiveMask);
}
